...
import { authClient } from "@/app/auth"; // confirm this path exports authClient
...
export default function LoginPage() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      // authClient.signIn.email may either throw (with fetchOptions.throw) or return a result
      const result = await authClient.signIn.email({
        email,
        password,
        fetchOptions: { throw: false }, // we handle both thrown and returned errors below
      });

      // if the client returns an error object
      if (result?.error) {
      ...
      // success — redirect to requested page or portal
      const redirectTo = (searchParams?.get("redirectTo") as string) ?? "/portal/home";
      router.replace(redirectTo);

    } catch (err: unknown) {
      // handle thrown errors (network, unexpected)
