...
export async function signUpWithEmail(
  ...
  if (!isValidScEduEmail(email))
    return { error: "Please use a valid USC email address." };
  ...
  if (!userId)
    return {
      error: "Account created but user id was missing. Please contact support.",
    };
  ...
  try {
    ...
    console.error("failed to create accounts row after neon auth signup", {
      userId,
      email,
      e,
    });
  ...
  try {
    ...
    console.error(err);
    console.error("failed to create application row ");
    return {
      error:
        "your account was created but we could not create your application please contact support.",
    };
  }
