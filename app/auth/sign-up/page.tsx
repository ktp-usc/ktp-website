...
const PUBLIC_SIGN_UP_ENABLED =
  process.env.NEXT_PUBLIC_ENABLE_PUBLIC_SIGN_UP === "1";
...
export default function SignUpPage() {
  ...
  const handleSubmit = (event: FormEvent<HTMLFormElement>) => {
    ...
    if (signUpsClosed) {
      ...
      setErrorMessage(
        "Public account creation is disabled at the moment. Please check back later or contact the executive board if you believe this is an error.",
      );
      return;
    }
    ...
    if (!passwordMeetsRequirements(password)) {
      event.preventDefault();
      setErrorMessage(
        "Password does not meet requirements. Please ensure password is 8 characters long, contains at least one uppercase letter, one lowercase letter, one number, and one symbol.",
      );
    }
  };
  ...
  const blockForm =
    signUpsClosed || session.isLoading || Boolean(session.data?.user);
            ...
            {signUpsClosed ? (
              <>
                ...
                <p className="text-gray-600 dark:text-gray-400 mb-6">
                  Public account creation is disabled at the moment. Please
                  check back later or contact the executive board if you believe
                  this is an error.
                </p>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-xs text-gray-500 dark:text-gray-400">
                      Please ensure password is 8 characters long, contains at
